...
# Copy only the Cargo manifest files to leverage Docker caching
COPY Cargo.toml Cargo.lock ./

# Fetch dependencies without building the source
RUN cargo fetch
...
RUN cargo build --locked --release && \
    cp ./target/release/project-rust /bin/server
